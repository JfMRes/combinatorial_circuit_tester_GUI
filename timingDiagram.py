...
def showDiagram(path_in,salidas):
    ...
    with open(path,'r') as archivo:
        ...
        column_label=[]
        column_label.append("In")
        for i in range(bits_in-1,-1,-1):
            column_label.append("I"+str(i))
        column_label.append("->")
        
        for i in range(bits_out-1,-1,-1):
            column_label.append("O"+str(i))
        column_label.append("Out")
        ...
        ax2.axis('tight')
        ax2.axis('off')
        fig2.suptitle("True table:  "+path.split("/")[-1].split(".")[0],ha='left')
        


        ax2.table(cellText=table_list,colLabels=column_label,loc='upper center').auto_set_font_size(False)

        
        imagefile=path_in.split("/")[-1].split(".")[0]+".jpeg"
